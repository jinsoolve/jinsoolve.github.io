...
## 풀이
한 학생을 기준으로 해당 아는 사이가 아닌 학생들은 서로 같은 그룹이어야 한다.  
이를 union-find를 사용하여 그룹을 묶는다.  

위 그룹이 서로 모두 아는 사이인지 확인한다.  
이때, 해당 그룹은 여러개가 나올 수 있는데 1번 학생이 포함된 그룹에 포함시킬 수 있는 만큼 포함시키고 나머지는 다른 그룹에 합친다. 다른 그룹들에 대해서 모두 아는 사이인지 확인하면 된다.  
